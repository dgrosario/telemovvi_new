...
export const SubflowNode = memo(({ id, data, selected }: NodeProps) => {
  const highlighted = data?.highlighted;

  const getStatusText = () => {
    if (data.targetFlowName) return data.targetFlowName;
      ...
      <Box sx={nodeContainerStyle}>
        ...
        >
          ...
          >
            ...
            />
            <AccountTreeIcon sx={{ color: "white", fontSize: 44 }} />
            <Handle
              type="source"
        ...
        <NodeDeleteButton nodeId={id} nodeType="subflow" />
      </Box>
    </Fade>
  );
});
