...
export const InputNode = memo(({ id, data, selected }: NodeProps) => {
  ...
  const getContentPreview = () => {
    if (!data.question) return "Não configurado";
    return data.question.length > 25
      ? data.question.substring(0, 25) + "..."
      : data.question;
  };
      ...
      <Box sx={nodeContainerStyle}>
        ...
        >
          ...
          >
            ...
            />
            <HelpOutlineIcon sx={{ color: "white", fontSize: 44 }} />
            <Handle
              type="source"
        ...
        <NodeDeleteButton nodeId={id} nodeType="input" />
      </Box>
    </Fade>
  );
});
