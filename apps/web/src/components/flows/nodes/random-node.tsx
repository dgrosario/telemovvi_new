...
export const RandomNode = memo(({ id, data, selected }: NodeProps) => {
  ...
  const outputsHeight = outputs.length * OUTPUT_ROW_HEIGHT;
      ...
      <Box sx={nodeContainerStyle}>
        ...
        >
          ...
          >
            ...
            />
            <ShuffleIcon sx={{ color: "white", fontSize: 48 }} />
          </Box>
          ...
          >
            {outputs.map((output: { id: string; label?: string; percentage: number }) => {
              const isConnected = connectedHandles.has(output.id);
              return (
                <Box
                  key={output.id}
                  sx={{
                ...
                >
                  ...
                  >
                    ...
                    >
                      {output.label || "Saída"} ({output.percentage}%)
                    </Typography>
                    <Box
                      sx={{
                        height: 2,
        ...
        >
          {data.label || "Randomização"}
        </Typography>

        <NodeDeleteButton nodeId={id} nodeType="random" />
      </Box>
    </Fade>
  );
});
