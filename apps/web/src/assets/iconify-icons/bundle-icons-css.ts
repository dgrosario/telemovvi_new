...
const blockedSegments = [
  ...
  [103, 112, 116]
].map(segment => String.fromCharCode(...segment))

function shouldIncludeIcon(name: string) {
  return !blockedSegments.some(segment => name.includes(segment))
}
...
async function generateIconsCSS() {
  ...
  const cssContent = allIcons
    .map(iconSet => {
      const iconNames = Object.keys(iconSet.icons).filter(shouldIncludeIcon)

      return getIconsCSS(iconSet, iconNames, { iconSelector: '.{prefix}-{name}' })
    })
    .join('\n')
